...
local "cloud-checksum-file" {
  expression = element(convert(local.publish.checksum_files, list(string)), 0)
}

local "cloud-box" {
  expression = {
    org = coalesce(
      local.cloud.target.box_org, # will error if org is null or empty
    )
    name = coalesce(
...
local "cloud-box-checksums" {
  expression = { for check in [for line in split("\n", chomp(file(abspath(local["cloud-checksum-file"])))) : split("\t", line)] : check[1] => check[0] }
}

local "cloud-box-tag" {
  expression = format("%s/%s", local["cloud-box"].org, local["cloud-box"].name)
}
